...
import {
  bookTicketsAction,
  getBookedSeatsAction,
} from "@/actions/ticket-booking";
...
const convertSeatPrices = (pricesArray) => {
  ...
  pricesArray.forEach((priceItem) => {
    ...
    seatTypesObject[typeKey] = {
      ...
      rows: defaultRowDistribution[typeKey]
        ? defaultRowDistribution[typeKey].rows
        : [],
    };
  });
...
const DEFAULT_SEAT_TYPES = {
  standard: { name: "Standard", price: 150.0, rows: [0, 1, 2] },
  vip: { name: "VIP", price: 350.0, rows: [3, 4, 5] },
  premium: { name: "Premium", price: 250.0, rows: [6, 7] },
};
...
const TicketBookingPage = ({
  ...
  const stableLayout = useMemo(
    () => layout,
    [layout.rows, layout.seatsPerRow, layout.aislePosition]
  );

  // 1. 🛠️ FIX: Stabilize seatTypes reference using useRef/useMemo
  // This ensures that createSeatMap doesn't re-create itself on every render.
  const previousPricesParamRef = useRef(pricesParam);
  const seatTypesRef = useRef(DEFAULT_SEAT_TYPES);

  const seatTypes = useMemo(() => {
    // If param string is the same as the previous render, return the cached stable object.
    if (pricesParam === previousPricesParamRef.current) {
      return seatTypesRef.current;
    }
    ...
    if (pricesParam) {
      try {
        ...
        result = convertSeatPrices(seatPrices);
      } catch (error) {
        console.error(
          "Error processing seat prices from URL. Falling back.",
          error
        );
        result = DEFAULT_SEAT_TYPES;
      }
    } else {
      result = DEFAULT_SEAT_TYPES;
    }

    // Update the cache for stability on subsequent renders
    previousPricesParamRef.current = pricesParam;
    seatTypesRef.current = result;

    return result;
  }, [pricesParam]); // fetching booked seats
  // --- End Stability Fix for seatTypes ---
  ...
  const colors = COLORS;
  const bookedSeatsSet = useMemo(() => new Set(bookedSeats), [bookedSeats]); // 2. 🛠️ FIX: createSeatMap is stable only if its dependencies are stable.

  const createSeatMap = useCallback(
    (currentBookedSeats) => {
      const seats = [];
      for (let row = 0; row < layout.rows; row++) {
        const seatRow = []; // seatTypes is now guaranteed stable if pricesParam didn't change
        ...
        for (let seat = 0; seat < layout.seatsPerRow; seat++) {
          ...
          seatRow.push({
            ...
            price: seatTypeInfo?.price || 150.0,
            color: seatTypeInfo?.color || "blue",
            status: currentBookedSeats.has(seatId) ? "Booked" : "Available",
            selected: false,
          });
        }
        seats.push(seatRow);
    ...
    [stableLayout, seatTypes, colors]
  );

  // 3. 🛠️ FIX: Initialize seats using a function to call createSeatMap immediately.
  const [seats, setSeats] = useState(() => createSeatMap(bookedSeatsSet));

  // 4. 🛠️ FIX: The problematic useEffect (line 185) now relies on stable dependencies.
  useEffect(() => {
    setSeats(createSeatMap(bookedSeatsSet));
    // This effect only runs when bookedSeatsSet (data) changes or createSeatMap (config/logic) changes.
    // Since createSeatMap is now stable for constant configuration, the loop is broken.
  }, [bookedSeatsSet, createSeatMap]); // Memoize selected seats for summary calculation
  const selectedSeats = useMemo(
    () => seats.flat().filter((s) => s.selected),
    [seats]
  );
  ...
  const ticketWord = ticketCount === 1 ? "Ticket" : "Tickets"; // --- Styling Logic ---
  ...
  const getSeatClassName = useCallback(
    (seat) => {
      const baseClass =
        "w-8 h-8 sm:h-10 lg:w-12 lg:h-12 m-1 rounded-t-lg border-2  transition-all duration-200 flex items-center justify-center text-xs sm:text-sm font-bold shadow-md";
  ...
  const handleBookingComplete = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(result.message); // Clear selections on the current seat map
        setSeats((prevSeats) =>
          prevSeats.map((r) => r.map((s) => ({ ...s, selected: false })))
        ); // Trigger an update to bookedSeats, which in turn triggers the useEffect to re-render the map with 'Booked' status
    ...
    <div className="mt-8 pt-4 border-t border-gray-200">
      ...
      <div className="flex flex-wrap justify-center space-x-6 text-sm">
        ...
        {Object.entries(seatTypes).map(([typeKey, config], index) => {
          ...
          return (
            <div key={typeKey} className="flex items-center m-2">
              <span
                className={`w-5 h-5 rounded-full mr-2 border-2 ${getColorClass(
                  colorName
                )}`}
              ></span>
        ...
        <div className="flex items-center m-2">
          <span className="w-5 h-5 rounded-full mr-2 bg-gray-400 border-gray-500"></span>{" "}
          Booked
        </div>
        <div className="flex items-center m-2">
          <span className="w-5 h-5 rounded-full mr-2 bg-green-500 border-green-700"></span>{" "}
          Selected
        </div>
      </div>
    </div>
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 p-4 font-sans">
      <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-2xl p-6 md:p-10 border border-gray-100">
        ...
        <p className="text-center mb-10 uppercase text-gray-700 font-semibold text-sm mt-3">
          SCREEN THIS WAY
        </p>
        ...
        <div className="mt-10 pt-6 border-t border-gray-200">
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap" style={{ minHeight: "32px" }}>
              {selectedSeats.map((s) => (
                <span
                  key={s.id}
                  className="bg-indigo-100 text-indigo-700 text-xs font-semibold px-2 py-1 rounded-full m-1 border border-indigo-300"
                >
                  {s.id} ({s.type})
                </span>
              ))}
              {selectedSeats.length === 0 && (
                <p className="text-gray-400 italic m-1">
          ...
          >
            {isBooking ? (
              ...
              `Book ${
                selectedSeats.length ? selectedSeats.length : "your"
              } ${ticketWord}`
            )}
          </button>
        </div>
